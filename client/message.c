...
unsigned char *createMessage(unsigned char *mess, int mess_len, unsigned char* key, int key_len, int* tot_len){
    ...
    if (result == NULL) {
        perror("[Client] Errore allocazione memoria in createMessage");
        *tot_len = 0;
        return NULL;
    }
    ...
    *tot_len = total_length;
    ...
    printf("[Client] Messaggio creato - Lunghezza totale: %d\n", total_length);
    printf("[Client] Lunghezza messaggio: %s\n", m_len);
    printf("[Client] Lunghezza chiave: %d\n", key_len);
...
message *readMessage(unsigned char* string) {
    ...
    if (m == NULL) {
        perror("[Client] Errore allocazione memoria in readMessage");
        return NULL;
    }
    ...
    if (second_last_null == -1 || last_null == -1) {
        printf("[Client] Errore: formato messaggio non valido\n");
        free(m);
        return NULL;
    }
    ...
    printf("[Client] Messaggio letto (NUOVO APPROCCIO):\n");
    printf("[Client] Lunghezza: %d\n", m->length);
    printf("[Client] Chiave: %s\n", m->key);
    printf("[Client] Posizione secondo_ultimo_null: %d\n", second_last_null);
    printf("[Client] Posizione ultimo_null: %d\n", last_null);
...
message *readMessageWithLength(unsigned char* string, int total_len) {
    ...
    if (second_last_null == -1 || last_null == -1) {
        printf("[Client] Errore: formato messaggio non valido (lunghezze: %d, %d)\n", second_last_null, last_null);
        free(m);
        return NULL;
    }
    ...
    printf("[Client] Messaggio letto con lunghezza %d:\n", total_len);
    printf("[Client] Lunghezza messaggio: %d\n", m->length);
    printf("[Client] Chiave: %s\n", m->key);
    printf("[Client] Separatori trovati alle posizioni: %d, %d\n", second_last_null, last_null);
